...
@if($v -> state == '2')
				...
				<td class="td-manage">
					<a style="text-decoration:none"  href="/admin/action?action=stateup&uid={{$v->uid}}" title="启用">	
@else
					<span class="label label-success radius">{{$state[$v -> state]}}</span>
					</td>
				<td class="td-manage">
					<a style="text-decoration:none" href="/admin/action?action=statedown&uid={{$v -> uid}}" title="停用">
@endif
					...
					<a title="删除" href="/admin/action?action=delete&uid={{$v -> uid}}" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e2;</i>
					</a>
				</td>
			</tr>
